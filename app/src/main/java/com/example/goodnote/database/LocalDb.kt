...
abstract class LocalDb : RoomDatabase() {
    ...
    companion object {
        ...
        private fun buildDb(context: Context): LocalDb {
            // weirdly didn't populate with .also{ }
            //return Room.databaseBuilder(context, LocalDb::class.java, DB_NAME).build().also { populate(it) }
            val db = Room.databaseBuilder(context, LocalDb::class.java, DB_NAME).build()
            populate(db)
            return db
        }

        private fun populate(db: LocalDb) {
            GlobalScope.launch {
                val noteDao = db.noteDao()
                val tagDao = db.tagDao()
                val joinDao = db.joinNoteTagDao()
